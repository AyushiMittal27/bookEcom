...
const Orders = ()=>{
    ...
    const handleStatusChange= (e, orderId)=>{
        updateOrderStatus(user._id,token, orderId, e.tatgetValue)
        .then((data)=>{
            if(data.error){
              console.log('Status updation failed', data.error)
            }else{
                loadOrders()
            }
        })
    ...
    return(
        <Layout
        title="Orders"
        description={`G'da ${user.name} , you can manage all orders here`}
        >
            <div className="row">
