...
exports.signin = (req,res)=>{
 ...
 User.findOne({email} , (err , user)=>{
     ...
     {
         return res.status(400).json({
             err : 'User with that email does not exits.Please signup using correct details'
         })
     }
     ...
     const token = jwt.sign({_id: user._id} , process.env.JWT_SECRET)
      //persist the token as 't' in cookiee with expiry date
...
exports.isAuth =(req, res, next)=>{
    ...
    console.log(req.profile ," and" , req.auth)
    if(!user){
